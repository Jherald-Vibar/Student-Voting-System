...
<div class="container mx-auto px-4 mt-5">
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mt-10">
        ...
        <div class="bg-white p-6 rounded-xl shadow hover:shadow-lg transition duration-300 flex flex-col justify-between h-full border-l-4 {{ $hasEnded ? 'border-red-500' : 'border-green-500' }}">
            ...
            <div class="flex justify-end gap-2 mt-auto pt-4 border-t border-gray-100">
                <a href="{{ route('election-edit', ['id' => $election->id]) }}"
                   class="px-4 py-2 bg-blue-800 text-white rounded-lg text-sm hover:bg-yellow-500 transition">
                    Edit
                </a>
                ...
                <form id="deleteElectionForm-{{ $election->id }}" action="{{ route('election-delete', ['id' => $election->id]) }}" method="POST" class="inline">
                    ...
                    @method('DELETE')
                    <button type="button"
                            class="px-4 py-2 bg-blue-800 text-white rounded-lg text-sm hover:bg-red-600 transition"
